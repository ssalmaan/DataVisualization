...
if version_info >= (3, 0, 0):
    new_instancemethod = lambda func, inst, cls: _ITKConnectedComponentsPython.SWIG_PyInstanceMethod_New(func)
else:
...
if version_info >= (2, 6, 0):
    def swig_import_helper():
        ...
        fp = None
        try:
            fp, pathname, description = imp.find_module('_ITKConnectedComponentsPython', [dirname(__file__)])
        except ImportError:
            import _ITKConnectedComponentsPython
            return _ITKConnectedComponentsPython
        if fp is not None:
            try:
                _mod = imp.load_module('_ITKConnectedComponentsPython', fp, pathname, description)
            finally:
                fp.close()
            return _mod
    _ITKConnectedComponentsPython = swig_import_helper()
