...
class GradientVolume:
    ...
    def compute(self):
        ...
        for x in range(1, self.volume.dim_x -1 , 1):
            for y in range(1, self.volume.dim_y - 1 , 1):
                for z in range(1,  self.volume.dim_z - 1, 1):
                    volgx = (self.volume.get_voxel(x+1,y,z) - self.volume.get_voxel(x-1,y,z)) / 2
                    volgy = (self.volume.get_voxel(x, y+1, z) - self.volume.get_voxel(x, y-1, z)) / 2
                    volgz = (self.volume.get_voxel(x, y, z+1) - self.volume.get_voxel(x, y, z-1)) / 2
                    volg=VoxelGradient(volgx,volgy,volgz)
                    self.set_gradient(x,y,z,volg)
