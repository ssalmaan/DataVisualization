...
if version_info >= (3, 0, 0):
    new_instancemethod = lambda func, inst, cls: _ITKSmoothingPython.SWIG_PyInstanceMethod_New(func)
else:
...
if version_info >= (2, 6, 0):
    def swig_import_helper():
        ...
        fp = None
        try:
            fp, pathname, description = imp.find_module('_ITKSmoothingPython', [dirname(__file__)])
        except ImportError:
            import _ITKSmoothingPython
            return _ITKSmoothingPython
        if fp is not None:
            try:
                _mod = imp.load_module('_ITKSmoothingPython', fp, pathname, description)
            finally:
                fp.close()
            return _mod
    _ITKSmoothingPython = swig_import_helper()
